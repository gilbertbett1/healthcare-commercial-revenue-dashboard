...
# Load CSV files into DataFrames and then into the database
...
with engine.connect() as conn:
    ...
    orphan_c = conn.execute(
        text("SELECT COUNT(*) FROM fact_sales f LEFT JOIN dim_clients c ON f.client_key=c.client_key WHERE c.client_key IS NULL")).scalar()
    orphan_p = conn.execute(
        text("SELECT COUNT(*) FROM fact_sales f LEFT JOIN dim_products p ON f.product_key=p.product_key WHERE p.product_key IS NULL")).scalar()
